...
def xavier_weight_init():
    ...
    def _xavier_initializer(shape, **kwargs):
        ...
        """
        # YOUR CODE HERE
        eps = np.sqrt(6. / np.sum(shape, dtype=np.float32))
...
def test_initialization():
    ...
    with tf.Session() as sess:
        xavier_mat_np = sess.run(xavier_mat)
    assert (xavier_mat_np >= -eps).all() and (xavier_mat_np <= eps).all()
    print "My Xavier initialization tests pass\n"
    # END YOUR CODE
