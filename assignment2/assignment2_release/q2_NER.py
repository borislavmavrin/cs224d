...
class NERModel(LanguageModel):
    ...
    def load_data(self, debug=False):
        ...
        self.X_test, self.y_test = du.docs_to_windows(
            docs, word_to_num, tag_to_num, wsize=self.config.window_size)
        # np.any(y_test) will return False, since all lables are zeros.
        # Make at least one entry non zeros
        self.y_test[0] = 1
    ...
    def add_embedding(self):
        ...
        with tf.device('/cpu:0'):
            # YOUR CODE HERE
            L = tf.Variable(tf.convert_to_tensor(self.wv,
                                                          dtype=tf.float32))
            window = tf.nn.embedding_lookup(L, self.input_placeholder)
            window = tf.reshape(window, shape=[-1,
    ...
    def add_model(self, window):
        ...
        Args:
          window: tf.Tensor of shape (-1, window_size*embed_size)
        Returns:
          y_hat: tf.Tensor of shape (batch_size, label_size)
        """
        # YOUR CODE HERE
        # try dropout both ways
        with tf.variable_scope("Layer"):
            xavier_initializer = xavier_weight_init()
            W = tf.get_variable(
                "weights",
                initializer=xavier_initializer([self.config.window_size
                                                * self.config.embed_size,
                                                self.config.hidden_size])
            )
            tf.add_to_collection('param_loss', 0.5 * self.config.l2 *
                                 tf.reduce_sum(W ** 2))
            b1 = tf.get_variable(
                "bias",
        ...
        with tf.variable_scope("Softmax"):
            ...
            scores = tf.matmul(h_drop, U) + b2
            scores_drop = tf.nn.dropout(scores, self.config.dropout)
        # END YOUR CODE
        return scores_drop
    ...
    def add_loss_op(self, scores):
        ...
        Args:
          scores: A tensor of shape (batch_size, n_classes). Unscaled log probs.
        Returns:
          loss: A 0-d tensor (scalar)
        """
        # YOUR CODE HERE
        loss = tf.nn.softmax_cross_entropy_with_logits(scores,
                                                       self.labels_placeholder)
        loss = tf.reduce_mean(loss)
        loss += tf.reduce_sum(tf.get_collection('param_loss'))
        # END YOUR CODE
        return loss
    ...
    def run_epoch(self, session, input_data, input_labels,
        ...
        for step, (x, y) in enumerate(
            ...
            feed = self.create_feed_dict(
                input_batch=x, dropout=dp, label_batch=y)
...
def calculate_confusion(config, predicted_indices, y_indices):
    ...
    confusion = np.zeros(
        (config.label_size, config.label_size), dtype=np.int32)
        ...
        with tf.Session() as session:
            ...
            for epoch in xrange(config.max_epochs):
                ...
                confusion = calculate_confusion(
                    config, predictions, model.y_dev)
